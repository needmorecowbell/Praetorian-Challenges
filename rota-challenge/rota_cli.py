...
def interactive(email, verbose=False):
    ...
    while (game_in_progress):
        ...
        try:
            if(result["status"]== "fail"): # There are multiple reason a query may fail..handle them here
                print("Query Failed")
                pprint(result)
                if("Active game not available" in result["data"]["request"] and game.is_active): # timeout check
                    choice= input("Game is inactive, new game? press y to continue: ")
                    if(choice is 'y'):
  ...
  *##           /         |         \           ##*
   *##      /             |             \      ##*
    *##  O                |                O  ##*
    ...
    parser.add_argument('-f', "--force", help="stall game to win every time",  action="store_true")
    ...
    if(args.force):
        if(args.verbose):
            print("Attempting to win Rota 50 times in a row...")
